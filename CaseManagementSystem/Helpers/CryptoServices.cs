...
namespace CaseManagementSystem.Helpers
{
    public interface ICryptoService
    {
        ...
        string EncryptStringEscaped(string text);
        string EncryptIdKey(int text);
        string EncryptIdKey(int text, string idKey);

        string DecryptString(string text);
        string DecryptStringEscaped(string text);
        int DecryptIdKey(string text, string idKey);
        int DecryptIdKey(string text);
    }

    public class CryptoService : ICryptoService
    {
        private const string _encKey = "8A9BE8FD67AF6979E7D20198CFEA50AA3D3733CCCAF2B72F";

        public string EncryptString(string text)
        {
            return Crypto.Encrypt(text, _encKey);
        }
        ...
        public string DecryptStringEscaped(string password)
        {
            return Crypto.DecryptEscaped(password, _encKey);
        }

        public string EncryptStringEscaped(string text)
        {
            return Crypto.EncryptEscaped(text, _encKey);
        }

        public string EncryptIdKey(int chyperText, string idKey)
        {
            return Crypto.EncryptEscaped(chyperText.ToString(), idKey);
        }

        public int DecryptIdKey(string chyperText, string idKey)
        {
            return Convert.ToInt32(Crypto.DecryptEscaped(chyperText, idKey));
        }

        public string EncryptIdKey(int text)
        {
            return this.EncryptIdKey(text, _encKey);
        }

        public int DecryptIdKey(string text)
        {
            return this.DecryptIdKey(text, _encKey);
        }
    }
    ...
    internal class Crypto
    {
        ...
        public static string EncryptEscaped(string clearText, string password)
        {
            string cipherText = Encrypt(clearText, password);
            cipherText = cipherText.Replace("+", "@").Replace("/", "_").Replace("=", "-");
            return cipherText;
        }

        public static string DecryptEscaped(string cipherText, string password)
        {
            return Decrypt(cipherText.Replace("@", "+").Replace("_", "/").Replace("-", "="), password);
        }
